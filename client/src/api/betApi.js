...
export const setTokens = (access, refresh) => {
  if (!ls) return;
  if (access) ls.setItem(ACCESS_KEY, access);
...
async function jsonOrThrow(res) {
  ...
  if (!res.ok || body.success === false) {
    ...
    if (
      res.ok &&
      (body.code === 'ODDS_CHANGED' || body.code === 'MARKET_CLOSED' || body.code === 'SELECTION_SUSPENDED')
    ) {
      err.status = 409;
    } else {
...
const del = (p, body) =>
  rawFetch(p, { method: 'DELETE', body: body ? JSON.stringify(body) : undefined }).then(jsonOrThrow);
