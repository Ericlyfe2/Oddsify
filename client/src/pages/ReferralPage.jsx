...
const SHARE_TARGETS = (link, msg) => [
  ...
  { label: 'Email', icon: '✉️', href: `mailto:?subject=${encodeURIComponent('Join me on BetNexa')}&body=${encodeURIComponent(`${msg}\n\n${link}`)}` },
];

const STATUS_STYLE = {
  pending: { label: 'Pending', color: '#f0a040', bg: 'rgba(240,160,64,.12)' },
  qualified: { label: 'Qualified', color: GOLD, bg: 'rgba(126, 217, 87,.12)' },
  rewarded: { label: 'Rewarded', color: GOLD, bg: 'rgba(126, 217, 87,.18)' },
...
export default function ReferralPage() {
  ...
  const shareMsg = 'Join BetNexa using my referral link and start winning today!';
            ...
            {Array.from({ length: 40 }).map((_, i) => (
              <span
                ...
                style={{
                  '--x': `${Math.random() * 100}%`,
                  '--d': `${Math.random() * 1.4}s`,
                  '--c': ['#a8e6a3', '#7ed957', '#fff3b8', '#d4a72c', '#f3e9cf'][i % 5],
...
@keyframes rfpGlow { 0%,100% { box-shadow: 0 0 0 1px rgba(126, 217, 87,.2) inset, 0 0 20px rgba(126, 217, 87,.04); } 50% { box-shadow: 0 0 0 1px rgba(168, 230, 163,.4) inset, 0 0 36px rgba(126, 217, 87,.1); } }
...
.rfp-badge { font-size: 10px; letter-spacing: .18em; font-weight: 800; color: ${GOLD}; background: rgba(126, 217, 87,.1); border: 1px solid rgba(126, 217, 87,.35); padding: 5px 10px; border-radius: 999px; }
.rfp-title { margin: 12px 0 6px; font-size: 26px; font-weight: 900; background: linear-gradient(110deg, #7ed957 25%, #fff7d6 50%, #7ed957 75%); background-size: 200% auto; -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; animation: rfpShimmer 3s linear 1s infinite; }
...
.rfp-card { background: radial-gradient(360px 140px at 80% -10%, rgba(126, 217, 87,.1), transparent 60%), linear-gradient(180deg, #161513 0%, #0f0e0c 100%); border-radius: 18px; padding: 16px; margin-bottom: 14px; box-shadow: 0 0 0 1px rgba(126, 217, 87,.15) inset; animation: rfpUp .4s ease .1s both; }
...
.rfp-btn-gold { background: linear-gradient(135deg, #a8e6a3 0%, #d4a72c 100%); color: #1a1300; box-shadow: 0 8px 20px rgba(126, 217, 87,.3); }
.rfp-btn-ghost { background: rgba(126, 217, 87,.06); color: ${CREAM}; border: 1px solid rgba(126, 217, 87,.2); }

.rfp-share { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 16px; }
.rfp-share-btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 12px; border-radius: 999px; background: rgba(126, 217, 87,.06); border: 1px solid rgba(126, 217, 87,.2); color: ${CREAM}; font-size: 12px; font-weight: 700; text-decoration: none; transition: background .15s, border-color .15s; }
.rfp-share-btn:hover { background: rgba(126, 217, 87,.14); border-color: rgba(126, 217, 87,.4); }

.rfp-stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 14px; }
.rfp-stat { background: rgba(126, 217, 87,.05); border: 1px solid rgba(126, 217, 87,.1); border-radius: 12px; padding: 12px 10px; display: flex; flex-direction: column; gap: 4px; animation: rfpUp .4s ease both; }
...
.rfp-row { display: flex; align-items: center; gap: 10px; padding: 10px 12px; border-radius: 12px; background: rgba(126, 217, 87,.04); border: 1px solid rgba(126, 217, 87,.08); }
...
.rfp-celebrate-card { position: relative; width: min(340px, 100%); text-align: center; background: linear-gradient(180deg, #161513 0%, #0a0a0a 100%); border-radius: 20px; padding: 26px 22px; box-shadow: 0 24px 80px rgba(0,0,0,.6), 0 0 0 1px rgba(126, 217, 87,.25) inset; animation: rfpPop .45s cubic-bezier(.18,.88,.36,1.2) both; }
