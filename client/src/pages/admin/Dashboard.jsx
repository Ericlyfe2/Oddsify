  ...
  return (
    <>
      <header className="adm-page-head">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="adm-btn primary">
            <IconSparkles size={14} /> Ask AI insights
          </button>
        </div>
      </header>

      {err && (
        <div className="adm-card" style={{ borderColor: 'rgba(255,93,108,.4)' }}>
          {err}
        </div>
      )}
      ...
      <div className="adm-stat-grid">
        ...
        <Stat
          label="Bets · 24h"
          value={numFmt(data?.kpis?.bets24h)}
          icon={<IconLive size={16} />}
          accent="linear-gradient(135deg,#ffb547,#ff5d6c)"
          delta={{ direction: 'up', label: '3.7%' }}
        />
      </div>
      ...
      <div className="adm-stat-grid">
        ...
        <Stat
          ...
          value={numFmt(data?.kpis?.pendingWithdrawals)}
        ...
        <Stat
          ...
          value={numFmt((data?.kpis?.auditCritical24h || 0) + (data?.kpis?.auditWarning24h || 0))}
          icon={<IconShield size={16} />}
          accent="linear-gradient(135deg,#ff5d6c,#ff5fb1)"
        />
      </div>
      ...
      <div className="adm-grid cols-8-4">
        <Card
          title="Revenue & stake — last 30 days"
          subtitle="GGR vs gross stake, daily."
          pill={
        ...
        <Card title="System health" subtitle="Realtime platform vitals">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <HealthRow
              label="Odds feed"
              ok={data?.health?.oddsApi?.enabled ?? false}
              value={
                data?.health?.oddsApi?.enabled
                  ? `Live · ${data?.health?.oddsApi?.lastFetchAt || '—'}`
            ...
            <HealthRow
              label="SMTP"
              ok={data?.health?.smtp}
              value={data?.health?.smtp ? 'Outbound mail OK' : 'Console mode (dev)'}
            />
            <HealthRow
              label="Google OAuth"
              ok={data?.health?.google}
              value={data?.health?.google ? 'Configured' : 'Disabled'}
            />
            <div className="adm-kv">
              <dt>Uptime</dt>
              <dd>{prettyUptime(data?.health?.uptimeSec)}</dd>
              <dt>Memory</dt>
              <dd>{data?.health?.memoryMb ?? '—'} MB</dd>
              <dt>Online admins</dt>
              <dd>{numFmt(data?.kpis?.onlineAdmins)}</dd>
              <dt>Audit events</dt>
              <dd>{numFmt(data?.kpis?.auditTotal)}</dd>
            </div>
          </div>
        </Card>
      </div>
              ...
              series={[
                {
                  ...
                  label: 'Signups',
                  color: '#18f0a1',
                  data: (charts?.userGrowth || []).map((d) => ({ date: d.date, y: d.value })),
                },
              ]}
            />
          )}
      ...
      <div className="adm-grid cols-7-5">
        <Card
          ...
          subtitle="Latest user-side events"
          action={
            <Badge tone="info">
        ...
        >
          {!data?.recent?.length ? (
            <Empty title="No activity yet" subtitle="Once players sign in and place bets you'll see them here." />
          ) : (
            <div className="adm-list-feed">
              {data.recent.map((r, i) => (
                <div key={i} className="row">
                  <span
                    className={`dot ${r.kind?.includes('failed') ? 'danger' : r.kind?.includes('cash') ? 'success' : r.kind?.includes('admin') ? 'warn' : ''}`}
                  />
                  <div>
        ...
        <Card
          ...
          subtitle="Audit events flagged by AI / rules"
          action={
            <Badge tone="danger" dot>
              {data?.kpis?.auditCritical24h || 0} critical
            </Badge>
          }
        >
          {!data?.alerts?.length ? (
            <Empty title="No alerts" subtitle="The platform looks clean for the past 24h." />
          ) : (
            <div className="adm-list-feed">
              {data.alerts.map((a) => (
                <div key={a.id} className="row">
                  ...
                  <div>
                    ...
                    <div className="meta">
                      {a.target || '—'} · {a.meta?.signal || a.severity}
                    </div>
                  </div>
      ...
      <div style={{ color: 'var(--text-mute)', fontSize: 12, textAlign: 'right' }}>
        Snapshot generated {data?.generatedAt ? ago(data.generatedAt) : '—'}
        {refreshing && (
          <>
            {' '}
