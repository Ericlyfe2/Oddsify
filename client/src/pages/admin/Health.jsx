        ...
        <>
          <div className="adm-grid c4" style={{ marginBottom: 18 }}>
            <Card title="Uptime (24h)">
              <div style={{ fontSize: 28, fontWeight: 800 }}>{s ? `${s.uptimePct.toFixed(2)}%` : '—'}</div>
              <div style={{ color: 'var(--text-dim)', fontSize: 12 }}>Active minutes without 5xx errors</div>
            </Card>
          ...
          <div className="adm-grid c2" style={{ marginBottom: 18 }}>
            ...
            <Card
              ...
              subtitle="Last 24h"
              pill={
                <Badge tone={tone(s?.p95Ms, { warnAt: 500, dangerAt: 1500 })} dot>
                  p95 {fmtMs(s?.p95Ms)}
                </Badge>
              }
            >
            ...
            <Card
              ...
              subtitle="Last 24h"
              pill={
                <Badge tone={tone(s?.errorRate24h * 100, { warnAt: 1, dangerAt: 5 })} dot>
                  {s ? fmtPct(s.errorRate24h) : '—'}
                </Badge>
              }
            >
            ...
            <Card
              title="Odds-feed lag (ms)"
              subtitle="Last 24h · per-provider fetch"
              pill={
                <Badge tone={tone(s?.oddsP95Ms, { warnAt: 1500, dangerAt: 5000 })} dot>
                  <IconBot size={12} /> p95 {fmtMs(s?.oddsP95Ms)}
                </Badge>
              }
            >
          ...
          <div className="adm-grid c2">
            <Card title="Runtime">
              <dl className="adm-kv">
                <dt>Uptime</dt>
                <dd>{r ? `${Math.floor(r.uptimeSec / 60)} min` : '—'}</dd>
                <dt>Memory (RSS)</dt>
                <dd>{r?.memoryMb ?? '—'} MB</dd>
                <dt>Node</dt>
                <dd>{r?.nodeVersion || '—'}</dd>
                ...
                <dd>{r?.pid ?? '—'}</dd>
                <dt>SMTP</dt>
                <dd>{r?.smtp ? <Badge tone="success">Configured</Badge> : <Badge tone="warn">Console</Badge>}</dd>
                <dt>Google OAuth</dt>
                <dd>{r?.google ? <Badge tone="success">On</Badge> : <Badge>Off</Badge>}</dd>
              </dl>
            </Card>
            <Card title="Odds API">
              <dl className="adm-kv">
                ...
                <dd>{r?.oddsApi?.requestsRemaining ?? '—'}</dd>
                <dt>Requests used</dt>
                <dd>{r?.oddsApi?.requestsUsed ?? '—'}</dd>
                <dt>Cache entries</dt>
                <dd>{r?.oddsApi?.cacheSize ?? '—'}</dd>
                <dt>Cache TTL</dt>
                <dd>{r?.oddsApi?.ttlMinutes ?? '—'} min</dd>
              </dl>
            </Card>
          </div>
          ...
          <div style={{ marginTop: 14, color: 'var(--text-dim)', fontSize: 12 }}>
            ...
            <span style={{ marginLeft: 4 }}>
              <IconLive size={12} /> Window: {data.windowMinutes} min · Bucket:{' '}
              {Math.round((data.bucketMs || 60000) / 1000)} s
            </span>
