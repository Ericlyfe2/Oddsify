...
export default function PromotionsAdmin() {
  ...
  async function load() {
    try {
      const r = await adminListPromotions();
  ...
  async function toggleActive(p) {
    if (!isSuper) return;
    try {
      const r = await adminPatchPromotion(p.id, { active: !p.active });
      setPromos((ps) => ps.map((x) => (x.id === p.id ? r.promotion : x)));
    } catch (e) {
      showToast(e.message, 'error');
    }
  }
  async function destroy(p) {
    ...
    try {
      await adminDeletePromotion(p.id);
    ...
    <>
      <header className="adm-page-head">
        <div>
          <h1>Promotions</h1>
          <p>
            Build, schedule and target the offers shown on the storefront.{' '}
            {isSuper ? '' : 'Read-only — needs super admin to mutate.'}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {isSuper && (
            <button className="adm-btn primary" onClick={() => setCreateOpen(true)}>
              <IconSparkles size={14} /> New promotion
            </button>
          )}
        </div>
      </header>
    ...
    <article className="adm-card" style={{ padding: 0, overflow: 'hidden' }}>
      ...
      <div style={{ padding: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
          <Badge tone={p.active ? 'success' : 'default'} dot>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button className="adm-btn sm" disabled={!canMutate} onClick={onEdit} style={{ flex: 1 }}>
            Edit
          </button>
          <button className="adm-btn sm" disabled={!canMutate} onClick={onToggle}>
            {p.active ? 'Hide' : 'Show'}
          </button>
          <button className="adm-btn sm danger" disabled={!canMutate} onClick={onDelete}>
            Delete
          </button>
        </div>
        <div style={{ marginTop: 8, color: 'var(--text-mute)', fontSize: 11 }}>
          Updated {ago(p.updatedAt)} · order #{p.order ?? 0}
        </div>
      </div>
    </article>
  );
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              ...
              <div className="adm-field">
                ...
                <input
                  ...
                  onChange={(e) => setForm((f) => ({ ...f, bonusRate: e.target.value }))}
                  placeholder="0.10 = 10%"
                />
              </div>
              <div className="adm-field">
                <label>Cap / user</label>
                <input
                  className="adm-input"
                  type="number"
                  value={form.capPerUser}
                  onChange={(e) => setForm((f) => ({ ...f, capPerUser: e.target.value }))}
                  placeholder="∞"
                />
              </div>
            </div>
          ...
          <div>
            ...
            >
              ...
              <div style={{ position: 'relative' }}>
                ...
                <h3 style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-.015em', margin: '0 0 6px' }}>
                  {form.title || 'Your promo title'}
                </h3>
                <p style={{ color: 'rgba(255,255,255,.85)', margin: '0 0 14px', fontSize: 13.5 }}>
                  {form.body || 'Describe the offer here…'}
                </p>
                <button
                  type="button"
                  style={{
