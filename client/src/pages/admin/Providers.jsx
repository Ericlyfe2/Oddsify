    ...
    <>
      <header className="adm-page-head">
        <div>
          <h1>API providers</h1>
          <p>
            Real-time health of every external sportsbook/data integration. Latency, success rate, and last error per
            provider.
          </p>
        </div>
      ...
      {!data ? (
        ...
        <>
            ...
            {(data.providers || []).map((p) => {
              ...
              return (
                ...
                >
                  <dl className="adm-kv">
                    <dt>ID</dt>
                    <dd style={{ fontFamily: 'var(--ff-mono)', fontSize: 12 }}>{p.id}</dd>
                    <dt>Last success</dt>
                    <dd>
                      {p.lastSuccessAt ? ago(p.lastSuccessAt) : <span style={{ color: 'var(--text-mute)' }}>—</span>}
                    </dd>
                    <dt>Last error</dt>
                    <dd>
                      {p.lastErrorAt ? (
                        <>
                          <Badge tone="danger">{ago(p.lastErrorAt)}</Badge>{' '}
                          <span style={{ color: 'var(--text-dim)' }}>{p.lastError}</span>
                        </>
                      ) : (
          ...
          <div className="adm-grid cols-7-5">
            <Card
              title={logFilter ? `API call log — ${logFilter}` : 'API call log'}
              subtitle="Recent outbound calls to providers. Sensitive params are scrubbed."
              action={
                logFilter && (
                  <button className="adm-btn sm ghost" onClick={() => setLogFilter('')}>
                    Clear filter
                  </button>
                )
              }
            >
              {filteredLogs.length === 0 ? (
                <Empty
                  ...
                  subtitle="Provider keys are dormant or the aggregator hasn't ticked yet."
                />
              ) : (
                    ...
                    <tbody>
                      {filteredLogs.map((l) => (
                        <tr key={l.id}>
                          <td>{ago(l.at)}</td>
                          <td>
                            <Badge tone="brand">{l.provider}</Badge>
                          </td>
                          <td
                            style={{
                          ...
                          <td className="num">
                            <Badge
                              tone={
                                l.status >= 200 && l.status < 300 ? 'success' : l.status >= 400 ? 'danger' : 'default'
                              }
                            >
                              {l.status || '—'}
                            </Badge>
            ...
            <Card
              ...
              subtitle="Live admin events as they fire"
              pill={
            ...
            >
              {liveLines.length === 0 ? (
                <Empty title="No live events yet" subtitle="Place a bet or run a settlement to see events stream in." />
              ) : (
                    ...
                    <div key={i} className="row">
                      ...
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 13.5 }}>
                          {l.k === 'audit' ? l.action || 'audit' : l.k}
                        </div>
                        <div className="meta">
                          {l.actorId || l.userId || ''} {l.betId ? `· ${l.betId.slice(0, 18)}` : ''}
                        </div>
                      </div>
          ...
          <Card subtitle="" title="Adding more keys">
            ...
            <ul style={{ fontFamily: 'var(--ff-mono)', fontSize: 12.5, marginTop: 10, paddingLeft: 18 }}>
              <li>
                The Odds API — <strong>ODDS_API_KEY</strong>
              </li>
              <li>
                API Football — <strong>APIFOOTBALL_KEY</strong>
              </li>
              <li>
                SportMonks — <strong>SPORTMONKS_TOKEN</strong>
              </li>
              <li>
                SharpAPI — <strong>SHARPAPI_KEY</strong>
              </li>
              <li>
                SportsGameOdds — <strong>SPORTSGAMEODDS_KEY</strong>
              </li>
            </ul>
          </Card>
        </>
...
function Mini({ label, v }) {
  return (
    ...
    >
      <div style={{ fontSize: 10.5, color: 'var(--text-mute)', textTransform: 'uppercase', letterSpacing: '.14em' }}>
        {label}
      </div>
