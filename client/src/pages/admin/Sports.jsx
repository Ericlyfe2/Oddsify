...
export default function SportsAdmin() {
  ...
  useEffect(() => {
    load();
  }, [filters.q, filters.sport, filters.leagueId, filters.status]); // eslint-disable-line
    ...
    <>
      <header className="adm-page-head">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {selectedFixtures.size > 0 && (
            ...
            >
              ...
              {hasRole('odds_manager') && (
                <>
                  <button className="adm-btn adm-btn-sm" onClick={() => doBulkFixtures('suspend')}>
                    Suspend
                  </button>
                  <button className="adm-btn adm-btn-sm" onClick={() => doBulkFixtures('unsuspend')}>
                    Unsuspend
                  </button>
                  <button className="adm-btn adm-btn-sm" onClick={() => doBulkFixtures('mark-live')}>
                    Mark live
                  </button>
                  <button className="adm-btn adm-btn-sm" onClick={() => doBulkFixtures('mark-upcoming')}>
                    Mark upcoming
                  </button>
                  <button className="adm-btn adm-btn-sm" onClick={() => setBulkScoreOpen(true)}>
                    Set result
                  </button>
                </>
              )}
              <button className="adm-btn adm-btn-sm" onClick={() => setSelectedFixtures(new Set())}>
                Clear
              </button>
            </div>
          )}
          ...
          {hasRole('odds_manager') && (
            <button className="adm-btn" onClick={() => setLeagueOpen(true)}>
          ...
          {hasRole('odds_manager') && (
            <button className="adm-btn warn" onClick={settleEverything}>
                  ...
                  return (
                    <tr key={m.id} onClick={() => setSelected(m)} className={selected?.id === m.id ? 'selected' : ''}>
                      ...
                      <td>
                        ...
                        {status === 'finished' && (
                          <Badge tone="success">
                            Finished {m.scoreHome}-{m.scoreAway}
                          </Badge>
                        )}
      ...
      {bulkScoreOpen && (
        <Modal
          open
          title={`Set result for ${selectedFixtures.size} fixtures`}
          onClose={() => setBulkScoreOpen(false)}
          footer={
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button className="adm-btn" onClick={() => setBulkScoreOpen(false)}>
                Cancel
              </button>
              <button
                className="adm-btn primary"
                onClick={() =>
                  doBulkFixtures('set-result', {
        ...
        >
          <div className="adm-field">
            ...
            <input
              ...
              min="0"
              value={bulkScoreHome}
              onChange={(e) => setBulkScoreHome(e.target.value)}
            />
          </div>
          <div className="adm-field">
            ...
            <input
              ...
              min="0"
              value={bulkScoreAway}
              onChange={(e) => setBulkScoreAway(e.target.value)}
            />
          </div>
        </Modal>
      )}
...
function FixtureDrawer({ open, fixtureId, onClose, hasRole, showToast, onChange }) {
  ...
  useEffect(() => {
    if (open) reload(); /* eslint-disable-next-line */
  }, [open, fixtureId]);
    ...
    <Drawer
      ...
      footer={
        fx && hasRole('odds_manager') ? (
          <>
            {fx.adminCreated && (
              <button className="adm-btn ghost" onClick={deleteFx}>
                Delete
              </button>
            )}
            <button className="adm-btn warn" onClick={toggleSuspendAll}>
              <IconBan size={14} /> {fx.suspended ? 'Unsuspend' : 'Suspend all'}
            </button>
            <button className="adm-btn primary" onClick={() => setResultModal(true)}>
        ...
        <>
          <Card>
            <dl className="adm-kv">
              ...
              <dt>Sport · League</dt>
              <dd>
                {fx.sport} · {fx.leagueName || fx.leagueId}
              </dd>
              <dt>Kick-off</dt>
              <dd>
                {fx.day} {fx.kickoff}
              ...
              <dd>
                {fx.finished ? (
                  <Badge tone="success">
                    Finished {fx.scoreHome}-{fx.scoreAway}
                  </Badge>
                ) : fx.isLive ? (
                  <Badge tone="danger" dot>
                    Live {fx.minute || ''}
              ...
              {fx.suspended && (
                <>
                  <dt>Suspended</dt>
                  <dd>
                    <Badge tone="warn">All markets suspended</Badge>
                  </dd>
                </>
              )}
            </dl>
            {hasRole('odds_manager') && !fx.finished && (
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                ...
                <button className="adm-btn sm" onClick={resetOdds}>
              ...
              action={
                ...
                !fx.finished && (
                  <div style={{ display: 'flex', gap: 4 }}>
                    <button className="adm-btn sm warn" onClick={() => suspendMarket(mk)}>
                      <IconBan size={12} /> Suspend
                    </button>
                    <button className="adm-btn sm ghost" onClick={() => removeMarket(mk)}>
                      <IconClose size={10} />
                    </button>
                  </div>
                )
  ...
  return (
    ...
    >
      ...
      <div className="adm-modal-actions">
        ...
        <button className="adm-btn primary" onClick={() => onSubmit(h, a)}>
          Record & settle
        </button>
      </div>
    </Modal>
...
function Side({ label, value, onChange }) {
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <input
        className="adm-input"
        style={{ width: 80, textAlign: 'center', height: 44, fontSize: 22, fontWeight: 700 }}
        type="number"
        min="0"
        max="199"
        value={value}
  ...
  const pickPreset = (mk) => {
    ...
    const presets = {
      ...
      ML: {
        ...
        selections: [
          { key: '1', label: 'Home', odds: '2.10' },
          { key: '2', label: 'Away', odds: '1.70' },
        ],
      },
      TP: {
        name: 'Total Points',
        selections: [
          { key: 'Over', label: 'Over', odds: '1.90' },
          { key: 'Under', label: 'Under', odds: '1.90' },
        ],
      },
      HCAP: {
        name: 'Handicap',
        selections: [
          { key: '1H', label: 'Home', odds: '1.90' },
          { key: '2H', label: 'Away', odds: '1.90' },
        ],
      },
      CS: {
        name: 'Correct Score',
    ...
    >
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {selections.map((sel, i) => (
          <div
            key={i}
            style={{ display: 'grid', gridTemplateColumns: '80px 1fr 90px 30px', gap: 6, alignItems: 'center' }}
          >
            <input
              className="adm-input"
              placeholder="Key"
              value={sel.key}
              onChange={(e) => updSel(i, 'key', e.target.value)}
              required
            ...
            <input
              ...
              placeholder="Label"
              value={sel.label}
              onChange={(e) => updSel(i, 'label', e.target.value)}
            />
            <input
              className="adm-input"
              type="number"
              ...
              placeholder="Odds"
              value={sel.odds}
              onChange={(e) => updSel(i, 'odds', e.target.value)}
              required
        ...
        <button type="button" className="adm-btn sm" onClick={addSel}>
          + Add selection
        </button>
...
function CreateFixtureModal({ open, onClose, leagues, onCreated, showToast }) {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="New fixture"
      description="Create a custom fixture (e.g. friendlies, special events)."
    >
      <form onSubmit={submit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div className="adm-field">
          ...
          <select
            ...
            value={form.sport}
            onChange={(e) => setForm((f) => ({ ...f, sport: e.target.value, leagueId: '' }))}
          >
            <option value="football">Football</option>
            <option value="basketball">Basketball</option>
            <option value="tennis">Tennis</option>
        ...
        <details style={{ gridColumn: '1 / -1', fontSize: 13 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 10 }}>
            <div className="adm-field">
              <label>{form.sport === 'football' ? 'Home (1)' : 'Home'} odds</label>
              <input
                className="adm-input"
                type="number"
                step="0.01"
                min="1.01"
                value={form.homeOdds}
                onChange={(e) => setForm((f) => ({ ...f, homeOdds: e.target.value }))}
              />
            </div>
            {form.sport === 'football' && (
              <div className="adm-field">
                <label>Draw (X) odds</label>
                <input
                  className="adm-input"
                  type="number"
                  step="0.01"
                  min="1.01"
                  value={form.drawOdds}
                  onChange={(e) => setForm((f) => ({ ...f, drawOdds: e.target.value }))}
                />
              </div>
            )}
            <div className="adm-field">
              <label>Away {form.sport === 'football' ? '(2)' : ''} odds</label>
              <input
                className="adm-input"
                type="number"
                step="0.01"
                min="1.01"
                value={form.awayOdds}
                onChange={(e) => setForm((f) => ({ ...f, awayOdds: e.target.value }))}
              />
            </div>
          </div>
        </details>
        {(form.sport === 'football' || form.sport === 'basketball') && (
          <details style={{ gridColumn: '1 / -1', fontSize: 13 }} open>
            <summary style={{ cursor: 'pointer', fontWeight: 600, color: 'var(--text-soft)' }}>
              Additional market odds
            </summary>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 10 }}>
              <div className="adm-field">
                <label>Over {form.sport === 'basketball' ? 'points' : '2.5'}</label>
                <input
                  className="adm-input"
                  type="number"
                  step="0.01"
                  min="1.01"
                  value={form.ouOver}
                  onChange={(e) => setForm((f) => ({ ...f, ouOver: e.target.value }))}
                />
              </div>
              <div className="adm-field">
                <label>Under {form.sport === 'basketball' ? 'points' : '2.5'}</label>
                <input
                  className="adm-input"
                  type="number"
                  step="0.01"
                  min="1.01"
                  value={form.ouUnder}
                  onChange={(e) => setForm((f) => ({ ...f, ouUnder: e.target.value }))}
                />
              </div>
              {form.sport === 'football' && (
                <>
                  <div className="adm-field">
                    ...
                    <input
                      ...
                      min="1.01"
                      value={form.bttsYes}
                      onChange={(e) => setForm((f) => ({ ...f, bttsYes: e.target.value }))}
                    />
                  </div>
                  <div className="adm-field">
                    <label>BTTS No</label>
                    <input
                      ...
                      min="1.01"
                      value={form.bttsNo}
                      onChange={(e) => setForm((f) => ({ ...f, bttsNo: e.target.value }))}
                    />
                  </div>
                  <div className="adm-field">
                    <label>DC 1X (Home/Draw)</label>
                    <input
                      className="adm-input"
                      type="number"
                      step="0.01"
                      min="1.01"
                      value={form.dc1X}
                      onChange={(e) => setForm((f) => ({ ...f, dc1X: e.target.value }))}
                    />
                  </div>
                  <div className="adm-field">
                    <label>DC X2 (Draw/Away)</label>
                    <input
                      className="adm-input"
                      type="number"
                      step="0.01"
                      min="1.01"
                      value={form.dcX2}
                      onChange={(e) => setForm((f) => ({ ...f, dcX2: e.target.value }))}
                    />
                  </div>
                  <div className="adm-field">
                    <label>DC 12 (Home/Away)</label>
                    <input
                      className="adm-input"
                      type="number"
                      step="0.01"
                      min="1.01"
                      value={form.dc12}
                      onChange={(e) => setForm((f) => ({ ...f, dc12: e.target.value }))}
                    />
                  </div>
                </>
              )}
...
function CreateLeagueModal({ open, onClose, onCreated, showToast }) {
  ...
  useEffect(() => {
    if (open) setForm({ name: '', sport: 'football', region: '', countryMeta: '' });
  }, [open]);
  async function submit(e) {
    e.preventDefault();
    try {
      ...
      <form onSubmit={submit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <div className="adm-field">
          ...
          <input
            ...
            placeholder="europe / africa / americas"
          />
        </div>
        <div className="adm-field" style={{ gridColumn: '1 / -1' }}>
          ...
          <input
            className="adm-input"
            value={form.countryMeta}
            onChange={(e) => setForm((f) => ({ ...f, countryMeta: e.target.value }))}
            placeholder="GHA · MATCHWEEK 18"
          />
        </div>
        <div className="adm-modal-actions" style={{ gridColumn: '1 / -1' }}>
          <button type="button" className="adm-btn ghost" onClick={onClose}>
