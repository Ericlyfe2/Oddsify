  ...
  return (
    <>
      <header className="adm-page-head">
        ...
        <Badge tone="brand">
          <IconSparkles size={12} /> Module preview
        </Badge>
      </header>
      <div className="adm-grid c2">
        {items.map((it) => (
          <Card
            ...
            pill={it.pill || <Badge tone="info">Coming</Badge>}
          >
            <ul
              style={{
                margin: 0,
...
export function AuditLogsPage() {
  ...
  useEffect(() => {
    adminAudit({ severity, action, limit: 200 })
            ...
            <tbody>
              ...
              {entries?.map((e) => (
                <tr key={e.id}>
                  ...
                  <td>
                    {e.actorId || '—'}{' '}
                    <span style={{ color: 'var(--text-dim)' }}>{e.actorRole ? `(${e.actorRole})` : ''}</span>
                  </td>
      ...
      <div className="adm-grid c2">
        <Card title="Runtime">
          <dl className="adm-kv">
            ...
            <dd>{health?.pid ?? '—'}</dd>
            <dt>SMTP</dt>
            <dd>{health?.smtp ? <Badge tone="success">Configured</Badge> : <Badge tone="warn">Console mode</Badge>}</dd>
            <dt>Google OAuth</dt>
            <dd>{health?.google ? <Badge tone="success">On</Badge> : <Badge>Off</Badge>}</dd>
            <dt>Odds feed</dt>
            <dd>{health?.oddsApi?.enabled ? <Badge tone="success">Live</Badge> : <Badge tone="warn">Cached</Badge>}</dd>
          </dl>
        </Card>
        <Card title="Platform toggles" subtitle="Affect the player site immediately">
          {[
            ...
            >
              ...
              <Badge tone={v ? 'success' : 'default'} dot>
                {v ? 'On' : 'Off'}
              </Badge>
            </div>
          ))}
        ...
        <Card title="Limits & compliance">
          <dl className="adm-kv">
            ...
            <dt>Max stake</dt>
            <dd>GHS 50,000</dd>
            <dt>Cash-out factor</dt>
            <dd>0.60×</dd>
            <dt>Bonus rate</dt>
            <dd>8% on multi-bets</dd>
            <dt>KYC threshold</dt>
            <dd>GHS 2,500 withdrawals</dd>
            <dt>Self-exclusion</dt>
            <dd>Available</dd>
          </dl>
        </Card>
      </div>
      ...
      >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              <input
                className="adm-input"
                readOnly
                value={lastIssued.signupUrl}
                style={{ fontFamily: 'var(--ff-mono)', fontSize: 12 }}
                onFocus={(e) => e.target.select()}
              />
              <button className="adm-btn" onClick={() => copy(lastIssued.signupUrl)}>
                Copy
              </button>
              <button className="adm-btn ghost" onClick={() => setLastIssued(null)}>
                Hide
              </button>
            </div>
        ...
        {invites && invites.length === 0 && (
          <Empty
            ...
            subtitle={isSuper ? 'Click "Issue invite" to create one.' : 'Only super admins can issue invites.'}
          />
        )}
        {invites && invites.length > 0 && (
              ...
              <tbody>
                {invites.map((inv) => (
                  <tr key={inv.id}>
                    <td>{inv.email}</td>
                    <td>
                      <Badge tone="brand">{inv.adminRole}</Badge>
                    </td>
                    <td>
                      {inv.status === 'pending' && (
                        <Badge tone="info" dot>
                    ...
                    <td className="row-actions">
                      {isSuper && inv.status === 'pending' && (
                        <button className="adm-btn sm danger" onClick={() => revoke(inv)}>
                          Revoke
                        </button>
                      )}
                    </td>
...
function CreateInviteModal({ open, onClose, onCreated }) {
  ...
  useEffect(() => {
    if (open) setForm({ email: '', adminRole: 'support', displayName: '', ttlDays: 7 });
  }, [open]);
    ...
    items={[
      {
        ...
        subtitle: 'CRUD on leagues, teams, fixtures.',
        bullets: [
          'Add / edit fixtures',
          'Schedule kick-offs',
          'Manage match metadata',
          'Mark fixtures live or settled',
        ],
      },
      {
        title: 'Market builder',
        subtitle: 'Compose markets and selections.',
        bullets: ['1X2 / OU / BTTS / handicap', 'Custom prop markets', 'Per-market limits', 'Liability caps'],
      },
      {
        title: 'Live odds control',
        subtitle: 'Real-time odds intervention.',
        bullets: ['Suspend single selection', 'Bulk freeze a fixture', 'Auto-rebalance liability', 'Trader notes'],
      },
      {
        title: 'Feed integration',
        subtitle: 'The Odds API + manual overrides.',
        bullets: ['Live feed status', 'Override values per league', 'Quality scores per provider', 'Failover routing'],
      },
    ]}
  />
);
    ...
    <>
      ...
      <div className="adm-grid c4" style={{ marginBottom: 18 }}>
        <Card title="Deposits (24h)">
          ...
          <div style={{ color: 'var(--text-dim)', fontSize: 12 }}>
            Lifetime {s ? moneyFmt(s.depositTotal) : '—'} · {s ? numFmt(s.depositCount) : '—'} txns
          </div>
        </Card>
        <Card title="Withdrawals (24h)">
          <div style={{ fontSize: 28, fontWeight: 800 }}>{s ? moneyFmt(s.sumOut24h) : '—'}</div>
          <div style={{ color: 'var(--text-dim)', fontSize: 12 }}>
            Lifetime {s ? moneyFmt(s.withdrawTotal) : '—'} · {s ? numFmt(s.withdrawCount) : '—'} txns
          </div>
        </Card>
        <Card title="Net cash">
          <div
            style={{
              fontSize: 28,
              fontWeight: 800,
              color: s && s.net >= 0 ? 'var(--success, #1aa46a)' : 'var(--danger, #d63a2c)',
            }}
          >
            {s ? moneyFmt(s.net) : '—'}
          </div>
      ...
      >
        ...
        {data && filtered.length === 0 && (
          <Empty title="No transactions yet" subtitle="They will appear here as players deposit and withdraw." />
        )}
        {data && filtered.length > 0 && (
              ...
              <tbody>
                {filtered.slice(0, 200).map((t) => (
                  <tr key={t.id}>
                    <td>{ago(t.at)}</td>
                    <td>
                      <Badge tone={t.kind === 'deposit' ? 'success' : 'warn'}>{t.kind}</Badge>
                    </td>
  ...
  return (
    <>
        ...
        <form onSubmit={send} style={{ display: 'grid', gap: 12 }}>
          <div className="adm-field">
            ...
            <input
              ...
              placeholder="e.g. Scheduled maintenance tonight at 23:00"
              required
              maxLength={80}
            />
          </div>
          <div className="adm-field">
            ...
            <textarea
              ...
              placeholder="Keep it short. Players see this in-app."
              required
              maxLength={500}
            />
          </div>
      ...
      <Card title="Recent broadcasts" subtitle={list ? `${list.length} total` : '—'}>
        {!list && <Spinner />}
        {list && list.length === 0 && (
          <Empty title="No broadcasts yet" subtitle="Anything you publish above will appear here." />
        )}
        {list && list.length > 0 && (
              ...
              <tbody>
                {list.map((n) => (
                  <tr key={n.id}>
                    ...
                    <td>
                      <Badge tone="info">{n.audience}</Badge>
                    </td>
                    <td>
                      <Badge
                        tone={
                    ...
                    <td className="row-actions">
                      <button className="adm-btn sm danger" onClick={() => remove(n.id)}>
                        Delete
                      </button>
                    </td>
                  </tr>
...
export function SupportPage() {
  ...
  const load = async () => {
    try {
      const r = await adminListTickets(statusFilter);
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid', gap: 16, gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)' }}>
        <Card
          title="Tickets"
          subtitle={tickets ? `${tickets.length} matching` : '—'}
          action={
        ...
        >
          ...
          {tickets && tickets.length === 0 && (
            <Empty title="No tickets" subtitle="When players send messages from /help they appear here." />
          )}
          {tickets && tickets.length > 0 && (
                ...
                <tbody>
                  {tickets.map((t) => (
                    <tr
                      ...
                      style={{
                        cursor: 'pointer',
                        background: active?.id === t.id ? 'var(--surface-soft, rgba(255,255,255,.04))' : '',
                      }}
                    >
                      <td>{ago(t.updatedAt)}</td>
                      <td>
                        {t.name}
                        {t.email ? <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>{t.email}</div> : null}
                      </td>
                      <td>{t.topic}</td>
                      <td>
                        <Badge tone={t.status === 'open' ? 'warn' : t.status === 'pending' ? 'info' : 'default'} dot>
                          {t.status}
                        </Badge>
                      </td>
                    </tr>
        ...
        <Card
          title={active ? `${active.topic} — ${active.name}` : 'Select a ticket'}
              ...
              {(active.replies || []).map((r, i) => (
                ...
                >
                  <div style={{ fontSize: 12, color: 'var(--text-dim)' }}>
                    <strong>{r.by}</strong> · {dateShort(r.at)}
                  </div>
              ...
              {active.status !== 'closed' && (
                <div style={{ display: 'grid', gap: 8 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                    ...
                    <button className="adm-btn primary" onClick={send} disabled={!reply.trim()}>
                      Send reply
                    </button>
                  </div>
                </div>
  ...
  return (
    <>
      <header className="adm-page-head">
        ...
        <Badge tone="warn">
          <IconBot size={12} /> Auto-refresh 30s
        </Badge>
      </header>

      <div className="adm-grid c4" style={{ marginBottom: 18 }}>
        <Card title="Flagged players">
          <div style={{ fontSize: 28, fontWeight: 800 }}>{data ? numFmt(data.counts.total) : '—'}</div>
        </Card>
        <Card title="High risk (≥60)">
          <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--danger, #d63a2c)' }}>
            {data ? numFmt(data.counts.high) : '—'}
          </div>
        </Card>
        <Card title="Medium (30–59)">
          <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--warn, #c87f00)' }}>
            {data ? numFmt(data.counts.medium) : '—'}
          </div>
        </Card>
        <Card title="Low (<30)">
          <div style={{ fontSize: 28, fontWeight: 800 }}>{data ? numFmt(data.counts.low) : '—'}</div>
        </Card>
      </div>
            ...
            <table className="adm-table">
              ...
              <tbody>
                {signals.map((s) => (
                  <tr key={s.userId}>
                    <td className="num">
                      <Badge tone={s.score >= 60 ? 'danger' : s.score >= 30 ? 'warn' : 'info'} dot>
                        {s.score}
                      </Badge>
                    </td>
                    <td>{s.email}</td>
