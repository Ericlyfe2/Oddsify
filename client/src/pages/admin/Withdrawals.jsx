...
import { adminListPendingWithdrawals, adminApproveWithdrawal, adminRejectWithdrawal } from '../../api/adminApi.js';
...
/**
 * Play a short attention chime when a new withdrawal request arrives. Uses
 * WebAudio so we don't ship an audio file; falls back to a no-op if the
 * browser blocks autoplay (admin gets the toast + OS notification anyway).
 */
function playChime() {
  try {
...
export default function WithdrawalsPage() {
  ...
  // Track which withdrawals we've already chimed for so a reload + socket
  // replay doesn't double-notify the admin for the same transaction.
  const seenIdsRef = useRef(new Set());

  const load = useCallback(async (opts = {}) => {
    try {
      const r = await adminListPendingWithdrawals();
      // Initial load seeds the seen-set so existing pending withdrawals
      // don't each fire a chime. Subsequent polls just keep the set in sync.
  ...
  // Live admin notifications when a user submits a new withdrawal. The
  // server emits `wallet:withdraw` to the admin namespace from
  // routes/wallet.js as soon as the pending row lands -- no need to wait
  // for the 8s poll.
  useEffect(() => {
    ...
    const off = onAdmin('wallet:withdraw', (payload) => {
      ...
      osNotify({ title, body, tag: `admin-withdrawal-${txId || Date.now()}` });
      playChime();
  ...
  const handleReject = async (id) => {
    ...
    // field, but the admin no longer has to fill it in to commit the
    // action. Native confirm guards against accidental clicks.
    if (!window.confirm('Reject this withdrawal? The held amount will be refunded to the user.')) return;
    setBusyId(id);
    try {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800 }}>Pending Withdrawals</h2>
          <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--text-dim)' }}>
            Approve or reject user withdrawal requests. Rejecting refunds the held amount. Refreshes every 8s.
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
        ...
        <div className="adm-stat">
          <div className="lbl">Total held (GHS)</div>
      ...
      <Card flush>
        {loading ? (
          ...
          <Empty title="No pending withdrawals" subtitle="All withdrawal requests have been processed." />
        ) : (
