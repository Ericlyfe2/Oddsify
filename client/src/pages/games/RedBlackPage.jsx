...
export default function RedBlackPage() {
  ...
  const play = (choice) => {
    if (!account) {
      ...
      navigate('/login?next=/casino/red-black');
      return;
    }
    ...
    setTimeout(() => {
      ...
      const drawn =
        Math.random() < RIG_LOSS_RATE ? (choice === 'red' ? 'black' : 'red') : Math.random() < 0.5 ? 'red' : 'black';
      setReveal(drawn);
    ...
    <div className="gp-shell">
      <div className="gp-frame">
        ...
        <div className="gp-stage" style={{ minHeight: 280 }}>
          ...
          {result === 'lose' && (
            <div className="gp-result lose">Card was {reveal?.toUpperCase()} — better luck next!</div>
          )}
        </div>
        ...
        <div className="gp-actions">
          <button type="button" className="gp-action red" disabled={!validBet || busy} onClick={() => play('red')}>
            {busy && pick === 'red' ? 'Flipping…' : 'RED'}
          </button>
          <button type="button" className="gp-action black" disabled={!validBet || busy} onClick={() => play('black')}>
