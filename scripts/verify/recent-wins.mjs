...
check(buckets.under5k > N * 0.6 && buckets.under5k < N * 0.8, '~70% under 5K');
check(buckets.mid > N * 0.15 && buckets.mid < N * 0.35, '~25% in 5K–50K');
check(buckets.big > N * 0.01 && buckets.big < N * 0.1, '~5% above 50K');
...
check(
  wins.every((x) => typeof x.phoneMasked === 'string'),
  'every item has phoneMasked',
);
check(
  wins.every((x) => ['real', 'synthetic'].includes(x.kind)),
