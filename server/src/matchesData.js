...
export function buildMarkets({ odds, ou = [1.85, 1.95], btts = [1.72, 2.05], dc }) {
  ...
  const p1hD = Math.max(0.3, pD + 0.18);
  ...
  return {
    ...
    WINBTTS: {
      ...
      selections: [
        { key: '1Y', label: 'Home & Yes', odds: priceFromProb(wbHy, 0.1) },
        { key: '1N', label: 'Home & No', odds: priceFromProb(wbHn, 0.1) },
        { key: 'XY', label: 'Draw & Yes', odds: priceFromProb(wbDy, 0.1) },
        { key: 'XN', label: 'Draw & No', odds: priceFromProb(wbDn, 0.1) },
        { key: '2Y', label: 'Away & Yes', odds: priceFromProb(wbAy, 0.1) },
        { key: '2N', label: 'Away & No', odds: priceFromProb(wbAn, 0.1) },
      ],
    },
    WINOU25: {
      name: 'Result & Total Goals (2.5)',
      selections: [
        { key: '1O', label: 'Home & Over 2.5', odds: priceFromProb(woHo, 0.1) },
        { key: '1U', label: 'Home & Under 2.5', odds: priceFromProb(woHu, 0.1) },
        { key: 'XO', label: 'Draw & Over 2.5', odds: priceFromProb(woDo, 0.1) },
        { key: 'XU', label: 'Draw & Under 2.5', odds: priceFromProb(woDu, 0.1) },
        { key: '2O', label: 'Away & Over 2.5', odds: priceFromProb(woAo, 0.1) },
        { key: '2U', label: 'Away & Under 2.5', odds: priceFromProb(woAu, 0.1) },
      ],
    },
    BTTSOU25: {
      name: 'BTTS & Total Goals (2.5)',
      selections: [
    ...
    HTFT: {
      ...
      selections: [
        { key: '1/1', label: 'Home / Home', odds: priceFromProb(htft['1/1'], 0.1) },
        { key: '1/X', label: 'Home / Draw', odds: priceFromProb(htft['1/X'], 0.1) },
        { key: '1/2', label: 'Home / Away', odds: priceFromProb(htft['1/2'], 0.1) },
        { key: 'X/1', label: 'Draw / Home', odds: priceFromProb(htft['X/1'], 0.1) },
        { key: 'X/X', label: 'Draw / Draw', odds: priceFromProb(htft['X/X'], 0.1) },
        { key: 'X/2', label: 'Draw / Away', odds: priceFromProb(htft['X/2'], 0.1) },
        { key: '2/1', label: 'Away / Home', odds: priceFromProb(htft['2/1'], 0.1) },
        { key: '2/X', label: 'Away / Draw', odds: priceFromProb(htft['2/X'], 0.1) },
        { key: '2/2', label: 'Away / Away', odds: priceFromProb(htft['2/2'], 0.1) },
      ],
    },
    CS: {
      name: 'Correct Score',
...
function makeBasketMatch(row) {
  ...
  const markets = {
    ...
    TP: {
      name: `Total Points (Over/Under ${row.line || 220.5})`,
      selections: [
...
const SEED_SLIP_LEGS = [{ matchId: 'gh-kot-hea', market: '1X2', key: '1' }];
...
export const CASINO_GAMES = [
  {
    ...
    hue: '#22c55e',
    route: '/casino/dice',
    tagline: 'Pick a target. Roll the dice.',
  },
  {
    id: 'spin2win',
    title: 'Spin2Win',
    ...
    hot: true,
    hue: '#c81e1e',
    route: '/casino/spin2win',
    tagline: 'Roulette-style number grid.',
  },
  {
    id: 'red-black',
    title: 'Red Black',
    ...
    rtp: 98.0,
    hue: '#0ea5e9',
    route: '/casino/red-black',
    tagline: 'Flip the card. Pays 2x.',
  },
];
...
export const PROMOTIONS = [
  {
    ...
    tag: 'New customers',
    cta: 'Claim now',
    body: 'Get a 200% boost on your first deposit. Maximum bonus GHS 1,000.',
    expires: '30 days from signup',
  },
  {
    id: 'multi-boost',
    title: 'Acca Boost up to 75%',
    tag: 'All customers',
    cta: 'Build a multi',
    body: 'Multi-bets of 5+ legs earn boosts on potential winnings — up to 75% extra.',
    expires: 'Ongoing',
  },
  {
    id: 'cashback',
    ...
    tag: 'Active bettors',
    cta: 'Opt in',
    body: 'Get 15% cashback on net losses every Monday — credited automatically.',
    expires: 'Weekly',
  },
  {
    id: 'free-spin',
    ...
    tag: 'Casino',
    cta: 'Deposit & claim',
    body: 'Deposit GHS 100 and get 50 free spins on Sweet Bonanza.',
    expires: 'This week',
  },
];
