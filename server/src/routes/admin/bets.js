...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (credit > 0) {
      ...
      pushTx(user.id, {
        kind: result === 'won' ? 'bet_won' : 'bet_void_refund',
        amount: credit,
        status: 'completed',
    ...
    audit(req, {
      action: `bet.settle.${result}`,
      target: bet.id,
      targetType: 'bet',
      severity: result === 'void' ? 'warning' : 'info',
      meta: { credit, reason, userId: user.id },
    });
...
router.post(
  '/:id/cancel',
  requireAdmin,
  requireRole('odds_manager', 'finance_admin', 'moderator'),
  ...
  asyncHandler(async (req, res) => {
    ...
    audit(req, {
      ...
      targetType: 'bet',
      severity: 'warning',
      meta: { refund, reason: req.body.reason },
    });
    res.json({ bet: enrich(updatedBet) });
  }),
);

router.post('/:id/note', requireAdmin, validate(z.object({ note: z.string().min(1).max(1000) })), (req, res, next) => {
  const bet = betsStore.get(req.params.id);
  if (!bet) return next(notFound('Bet not found'));
...
router.post(
  '/:id/delete',
  requireAdmin,
  requireRole('moderator', 'odds_manager', 'finance_admin'),
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const id of betIds) {
      try {
        ...
        if (!bet) {
          results.push({ betId: id, status: 'error', error: 'Not found' });
          continue;
        }
        if (action === 'settle') {
          if (bet.status !== 'open') {
            results.push({ betId: id, status: 'error', error: `Already ${bet.status}` });
            continue;
          }
          const user = getUserById(bet.userId);
          if (!user) {
            results.push({ betId: id, status: 'error', error: 'User not found' });
            continue;
          }
          const r = result || 'lost';
          let credit = 0;
          ...
          if (credit > 0) {
            ...
            pushTx(user.id, {
              kind: r === 'won' ? 'bet_won' : 'bet_void_refund',
              amount: credit,
              status: 'completed',
              balanceAfter: updatedUser.balance,
              ref: bet.id,
          ...
          if (bet.status === 'cancelled') {
            results.push({ betId: id, status: 'error', error: 'Already cancelled' });
            continue;
          }
          if (bet.status === 'cashed_out') {
            results.push({ betId: id, status: 'error', error: 'Cashed out' });
            continue;
          }
          const user = getUserById(bet.userId);
          if (!user) {
          ...
          const updatedBet = {
            ...
            cancelledAt: new Date().toISOString(),
            cancelledBy: req.admin.email,
            cancelReason: reason || 'Bulk cancel',
          };
          betsStore.set(bet.id, updatedBet);
          if (refund > 0) {
    ...
    audit(req, {
      action: `bet.bulk.${action}`,
      target: `bets:${betIds.length}`,
      targetType: 'bet',
      severity: 'warning',
      meta: { count: results.length, action },
    });
