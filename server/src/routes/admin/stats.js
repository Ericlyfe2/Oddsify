...
function flatTx() { return Object.values(txStore.all() || {}).flat(); }
...
router.get('/summary', requireAdmin, (req, res) => {
  ...
  const payouts = bets.filter((b) => b.status === 'won').reduce((s, b) => s + (b.potentialWin || 0), 0)
...
router.get('/sports', requireAdmin, (_req, res) => {
  ...
  for (const b of bets) {
    for (const leg of b.legs || []) {
      ...
      if (b.status === 'won')  { cur.win += b.potentialWin / Math.max((b.legs || []).length, 1); cur.won++; }
  ...
  const matrix = keys.map((k) => {
    ...
    for (let i = 0; i < weeks; i++) {
      ...
      const active = cohort.filter((u) =>
        bets.some((b) => b.userId === u.id &&
          new Date(b.placedAt).getTime() >= winStart &&
          new Date(b.placedAt).getTime() <  winEnd)
      ).length;
      row.retention.push({
        week: i,
        active,
