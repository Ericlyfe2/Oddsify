    ...
    stats: {
      ...
      payoutTotal: Number(
        bets
          ...
          .reduce((s, b) => s + (b.potentialWin || 0), 0)
          .toFixed(2),
      ),
      txCount: tx.length,
      depositTotal: Number(
      ...
      patch = {
        ...
        verificationHistory: [
          ...(u.verificationHistory || []),
          {
            ...
            by: req.admin?.email || req.admin?.id || 'admin',
      ...
      patch = {
        ...
        verificationHistory: [
          ...(u.verificationHistory || []),
          {
            at: new Date().toISOString(),
            by: req.admin?.email || req.admin?.id || 'admin',
            reason: reason || 'Verification removed by admin',
            method: 'admin',
          },
        ],
      };
...
router.patch(
  ...
  (req, res, next) => {
    ...
    audit(req, {
      ...
      target: u.id,
      targetType: 'user',
      meta: { status: req.body.status, note: req.body.note },
    });
    res.json({ user: expandUser(next_) });
  },
);
...
router.patch(
  ...
  (req, res, next) => {
    ...
    audit(req, {
      ...
      targetType: 'user',
      severity: 'warning',
      meta: { delta: req.body.delta, balanceAfter: newBal, reason: req.body.reason },
    });
...
router.post(
  '/:id/reset-password',
  requireAdmin,
  requireRole(), // super only (no allowed list = super-only since super always passes)
...
router.delete(
  '/:id',
  requireAdmin,
  requireRole(), // super only
  validate(z.object({ reason: z.string().max(500).optional() }).optional()),
  asyncHandler(async (req, res, next) => {
    const u = getUserById(req.params.id);
    if (!u) return next(notFound('User not found'));
    if (u.role === 'admin')
      return next(badRequest('Cannot delete an admin from this endpoint. Demote the role first.'));
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const rawId of req.body.ids) {
      ...
      if (!u) {
        results.skipped.push({ id: rawId, reason: 'not_found' });
        continue;
      }
      if (u.role === 'admin') {
        results.skipped.push({ id: u.id, reason: 'is_admin' });
        continue;
      }
      revokeAllForAccount(u.id);
      const allBets = betsStore.all() || {};
...
router.get('/:id/credentials', requireAdmin, requireRole(), (req, res, next) => {
  const u = getUserById(req.params.id);
  if (!u) return next(notFound('User not found'));
  ...
  const hashAlgo =
    hash.startsWith('$2a$') || hash.startsWith('$2b$') || hash.startsWith('$2y$') ? 'bcrypt' : hash ? 'unknown' : null;
