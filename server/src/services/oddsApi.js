...
export const ODDS_SPORT_KEYS = {
  football: [
    ...
    },
    {
      ...
      region: 'europe',
      countryMeta: 'ESP · JORNADA 26',
      crest: { style: 'background:linear-gradient(135deg,#ee8707,#d12028);color:#fff', label: 'LL' },
    },
    {
      ...
      region: 'europe',
      countryMeta: 'UEFA · ROUND OF 16',
      crest: { style: 'background:linear-gradient(135deg,#003366,#ffd100);color:#fff', label: 'CL' },
    },
    {
      ...
      region: 'europe',
      countryMeta: 'GER · SPIELTAG 27',
      crest: { style: 'background:linear-gradient(135deg,#d20515,#000);color:#fff', label: 'BUN' },
    },
    {
      ...
      leagueName: 'Serie A',
      region: 'europe',
      countryMeta: 'ITA · GIORNATA 28',
      crest: { style: 'background:linear-gradient(135deg,#008c45,#cd212a);color:#fff', label: 'SA' },
    },
  ],
  basketball: [
    {
    ...
    },
    {
      ...
      region: 'europe',
      countryMeta: 'EUR · ROUND 26',
      crest: { style: 'background:linear-gradient(135deg,#ff6b00,#000);color:#fff', label: 'EL' },
    },
  ],
  tennis: [
...
function buildBasketballMarkets(home, away) {
  return {
    ...
    TP: {
      name: 'Total Points (Over/Under 220.5)',
      selections: [
...
function buildTennisMarkets(home, away) {
  return {
    ...
    SETS: {
      name: 'Total Sets (Over/Under 2.5)',
      selections: [
...
function eventToMatch(ev, sport) {
  ...
  return {
    ...
    odds: sport === 'football' ? { 1: home, X: draw, 2: away } : { 1: home, 2: away },
    markets,
...
export async function fetchOddsForSportKey(sportKey, region = 'eu') {
  return cachedFetch(
    `${BASE}/sports/${encodeURIComponent(sportKey)}/odds/?regions=${region}&markets=h2h&oddsFormat=decimal&apiKey=${API_KEY}`,
  );
}
...
export async function fetchSportSnapshot(sport) {
  ...
  if (sport === 'tennis' && leagueDescriptors.length === 0) {
    ...
    leagueDescriptors = keys.map((s) => ({
      ...
      crest: {
        style: 'background:linear-gradient(135deg,#003c71,#fdb913);color:#fff',
        label: (s.title || 'TEN').slice(0, 3).toUpperCase(),
      },
    }));
  }

  const leagues = await Promise.all(
    leagueDescriptors.map(async (descriptor) => {
      let events = [];
      try {
